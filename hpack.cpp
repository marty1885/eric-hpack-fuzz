...
static const Integer2byte MaxOfInteger2byte = INT_FAST16_MAX;
static const unsigned char BitOfUinteger5Byte = (sizeof(Uinteger5byte) / sizeof(unsigned char)) * 8;

// need to keep order
static const unsigned char IndexedHeaderField = 0x80;
static const unsigned char LiteralHeaderFieldWithIncrementalIndexing = 0x40;
static const unsigned char DynamicTableSizeUpdate = 0x20;
static const unsigned char LiteralHeaderFieldNeverIndexed = 0x10;
static const unsigned char LiteralHeaderFieldWithoutIndexing = 0x00;

static const Integer2byte SymbolEOS = 256;
static const Integer2byte SymbolInitial = 257;
static const unsigned char HuffmanLiteralFlag = 0x80;
...
class HuffmanCode {
  public:
    ...
    HuffmanCode(const Uinteger5byte lsbvalue,const unsigned char lengthofbit):msbValue(lsbvalue << (BitOfUinteger5Byte - lengthofbit)),lengthOfBit(lengthofbit) {
        ;
    }
};
...
class HuffmanTableColumn {
  public:
    ...
    HuffmanTableColumn (const bool issymbol,const Integer2byte sym,const size_t huffmantable): isSymbol(issymbol), huffmanTable(huffmantable),symbol(sym) {
        ;
    }
};
...
class HuffmanTable {
  public:
    ...
    HuffmanTable(const unsigned char thisbit,const unsigned char plusbit,const std::vector<HuffmanTableColumn> &_table):tableAnd(((size_t)1 << plusbit) -1), thisBit(thisbit),table(_table) {
        ;
    }
};
...
class StaticTableMapElement {
  public:
    ...
    StaticTableMapElement(const std::string &_value,const size_t _index):index(_index), value(_value) {
        ;
    }
};

static const size_t StaticTableSize = 62;
static const std::pair<const std::string,const std::string> StaticTable[StaticTableSize] = {std::make_pair<const std::string, const std::string>("TableIndexZero", "TableIndexZero"),\
...
static const std::unordered_map<std::string,const std::vector<StaticTableMapElement> > StaticTableMap = {{":authority", {StaticTableMapElement("", 1)} },\
    {":method", {StaticTableMapElement("GET", 2),StaticTableMapElement("POST", 3)} },\
    {":path", {StaticTableMapElement("/", 4),StaticTableMapElement("/index.html", 5)} },\
    {":scheme", {StaticTableMapElement("http", 6),StaticTableMapElement("https", 7)} },\
    {":status", {StaticTableMapElement("206", 10),StaticTableMapElement("304", 11),StaticTableMapElement("400", 12),StaticTableMapElement("404", 13),StaticTableMapElement("500", 14),StaticTableMapElement("200", 8),StaticTableMapElement("204", 9)} },\
...
static const HuffmanCode HuffmanCodeTable[257] = {\
...
static const size_t HuffmanDecodeTableSize = 80;
static const HuffmanTable HuffmanDecodeTable[HuffmanDecodeTableSize] = {HuffmanTable(5, 5,{ \
...
inline const char* HeaderFieldRepr2CString(const HeaderFieldRepresentation &hfrp) {
    switch(hfrp) {
    case RInitial:
        return "Initial";
    ...
    case RLiteralHeaderFieldAlwaysIndex:
        return "AlwaysIndex";
    default:
        return "Unexpect status in HeaderFieldRepr2CString.";
    }
}

inline bool HeaderFieldInfoEqual(const HeaderFieldInfo& lhs,const HeaderFieldInfo& rhs) noexcept {
...
class VecHpWBuffer :public HpWBuffer {
  public:
    ...
    VecHpWBuffer(std::vector<unsigned char> &ss, size_t maxsize):maxSize(maxsize),s(ss) {
        ;
    }
...
#if ERICHPACKDEBUGFLAG == 1
    if (prefix > 8) {
        printf("IntegerEncode prefix(%jd) only supper less then 8.", (intmax_t)prefix);
    }
#endif // ERICHPACKDEBUGFLAG
...
#if ERICHPACKDEBUGFLAG == 1
    if (prefix > 8) {
        printf("IntegerDecode prefix(%jd) only supper less then 8.", (intmax_t)prefix);
    }
#endif // ERICHPACKDEBUGFLAG
  ...
  public:
    ...
    unsigned char ReadUntilBit(const HuffmanTable &hufftb, size_t &switchtableindex) {
        ...
        while(cacheBit < hufftb.thisBit) {
            ...
            tmp5byte_t = ((Uinteger5byte)r.Current()) << (BitOfUinteger5Byte - 8 - cacheBit);
            ...
            //printf("ReadUntilBit strpos(%llx) tmp5byte_t(%llx) huffmanCode(%llx) cacheBit(%u).\n", strpos, tmp5byte_t, huffmanCode, cacheBit);
        }

        switchtableindex = ((size_t)(huffmanCode >> (BitOfUinteger5Byte - hufftb.thisBit))) & hufftb.tableAnd;
        return CacheSuccess;
    }
    ...
    HpackStatus CheckStopAtStringLength() {
        ...
        if((cacheBit == 0) ? false : ((huffmanCode >> (BitOfUinteger5Byte - cacheBit)) != (tmp << cacheBit) - 1)) {
            return HpackStatus::LiteralPadNotEOSMsb;
        }
        return HpackStatus::Success;
    }
...
#if ERICHPACKDEBUGFLAG == 1
            default:
                printf("StringLiteralDecodeHuffman __HuffmanSwitch return unknown status(%u).\n", (unsigned int)rbcstat);
#endif // ERICHPACKDEBUGFLAG
            }
#if ERICHPACKDEBUGFLAG == 1
...
inline HpackStatus StringLiteralEncodeHuffman(HpWBuffer &w,const std::string &str) {
    ...
    if(bitsInBuffer != 0) {
        wa = (buffer >> (BitOfUinteger5Byte - 8)) | ((1 << (8 - bitsInBuffer)) - 1);
        if(!w.Append(wa)) {
            return HpackStatus::WBuffErrLiteralEecode;
        }
    }
...
class IndexTable {
  private:
    ...
    inline size_t DynamicTableEvict(const size_t evictnuntil) {
        size_t evictcn = 0;
        while(dynamicTableSize > evictnuntil) {
#if ERICHPACKDEBUGFLAG == 1
            if(dynamicTable.size() == 0 ) {
                printf("IndexTable::DynamicTableEvict dynamicTable and dynamicTableSize out of sync dynamicTable::size %zu dynamicTableSize %zu.\n", dynamicTable.size(), dynamicTableSize);
            }
#endif // ERICHPACKDEBUGFLAG
            evictcn +=1;
  ...
  public:
    const size_t* const dynamicTableMaxSize;
    IndexTable(const size_t* const dynamictablemaxsize):dynamicTableSize(0), dynamicTable() ,dynamicTableMaxSize(dynamictablemaxsize){
        ;
    }
    // Index start at 1
    ...
    inline size_t find(const KeyValPair &kvp, bool &onlymatchkey) {
        size_t indx = 0,i = 0;
        std::unordered_map<std::string,const std::vector<StaticTableMapElement> >::const_iterator iter;
    ...
    inline void DynamicTableInsert(const KeyValPair &kvp,size_t &evictcn) {
        ...
        evictcn = DynamicTableEvict(*dynamicTableMaxSize);
        return;
    }
    inline void DynamicTableSizeUpdate(const size_t newsize, size_t &evictcn) {
        evictcn = DynamicTableEvict(newsize);
        return;
    }
  ...
  public:
    ...
    inline HpackStatus _DecoderLoop(HpRBuffer &stream, KeyValPair &kvp, HeaderFieldInfo &hfdi) {
        ...
        if((stream.Current() & IndexedHeaderField) == IndexedHeaderField) {
            ...
            return HpackStatus::Success;
        } else if((stream.Current() & LiteralHeaderFieldWithIncrementalIndexing) == LiteralHeaderFieldWithIncrementalIndexing) {
            hfdi.representation = HeaderFieldRepresentation::RLiteralHeaderFieldWithIncrementalIndexing;
            stat = IntegerDecode(stream, 6, indx);
            INTEGERDECODEERRORCHECK(stat);
        } else if((stream.Current() & DynamicTableSizeUpdate) == DynamicTableSizeUpdate) {
            ...
            INTEGERDECODEERRORCHECK(stat);
            if(indx > *(indexTable.dynamicTableMaxSize)) {
#if ERICHPACKDEBUGFLAG == 1
                printf("_Hpack::_DecoderLoop TableSizeOverflow indexTable::dynamicTableMaxSize(%ju) indx(%ju).\n",(uintmax_t)(*(indexTable.dynamicTableMaxSize)), (uintmax_t)indx);
#endif // ERICHPACKDEBUGFLAG
                return HpackStatus::TableSizeOverflow;
            }
            indexTable.DynamicTableSizeUpdate(indx, hfdi.evictCounter);
            return HpackStatus::Success;
        } else if((stream.Current() & LiteralHeaderFieldNeverIndexed) == LiteralHeaderFieldNeverIndexed) {
            if(HFIF) {
    ...
    inline HpackStatus _Decoder(HpRBuffer &stream, std::vector<KeyValPair> &header, std::vector<HeaderFieldInfo> *headerfieldinfo) {
        ...
        do {
            if((stat = _DecoderLoop<HFIF>(stream,kvp, hfdi)) != HpackStatus::Success) {
#if ERICHPACKDEBUGFLAG == 1
            ...
            if (!HpackDecoderTableUpdateHeaderBlank && !HFIF) {
                if(hfdi.representation == HeaderFieldRepresentation::RDynamicTableSizeUpdate) {
                    hfdi.representation = HeaderFieldRepresentation::RInitial;
                } else {
    ...
    inline HpackStatus _EncoderLoopIndexed(HpWBuffer &stream,const size_t index) {
    ...
    template<StringLiteralEncodeWay SLEF>
    inline HpackStatus _EncoderLoopLiteralHeader(HpWBuffer &stream,const KeyValPair &kvp,const size_t index,HeaderFieldRepresentation hfrp) {
        HpackStatus stat;
    ...
    template<StringLiteralEncodeWay SLEF>
    inline HpackStatus _EncoderLoopAlwaysIndex(HpWBuffer &stream, const KeyValPair &kvp,const size_t index,const bool onlymatchkey, HeaderFieldInfo &hfdi) {
    ...
    inline HpackStatus _EncoderHeaderFieldInfo(HpWBuffer &stream,const std::vector<KeyValPair> &header,std::vector<HeaderFieldInfo> &hfdilst) {
        ...
        while(kvpiter != header.end()) {
            ...
            case HeaderFieldRepresentation::RLiteralHeaderFieldAlwaysIndex:
                ...
                break;
            case HeaderFieldRepresentation::RLiteralHeaderFieldWithIncrementalIndexing:
                stat = _EncoderLoopLiteralHeader<SLEF>(stream, *kvpiter, indx, HeaderFieldRepresentation::RLiteralHeaderFieldWithIncrementalIndexing);
                indexTable.DynamicTableInsert(*kvpiter, hfiter->evictCounter);
                break;
            case HeaderFieldRepresentation::RLiteralHeaderFieldWithoutIndexing:
            case HeaderFieldRepresentation::RLiteralHeaderFieldNeverIndexed:
    ...
    inline HpackStatus _Encoder(HpWBuffer &stream,const std::vector<KeyValPair> &header,const std::vector<std::string> *exclude,const StringLiteralEncodeWay SLEW) {
        switch(SLEW) {
        case StringLiteralEncodeWay::EOctet:
            return _Encoder<DOEXCLIDE, StringLiteralEncodeWay::EOctet>(stream, header, exclude);
        case StringLiteralEncodeWay::EHuffman:
            return _Encoder<DOEXCLIDE, StringLiteralEncodeWay::EHuffman>(stream, header, exclude);
        case StringLiteralEncodeWay::EShortest:
            return _Encoder<DOEXCLIDE, StringLiteralEncodeWay::EShortest>(stream, header, exclude);
        default:
            return HpackStatus::LiteralEncodeWay;
        }
    }
    inline HpackStatus _EncoderDynamicTableSizeUpdate(HpWBuffer &stream,const size_t newSize, HeaderFieldInfo &hfdi) {
        HpackStatus stat;
    ...
    _Hpack() :indexTable(&SETTINGS_HEADER_TABLE_SIZE_DEFAULT) {
        ;
    }
    _Hpack(const size_t* const settingheadertablemaxsize) :indexTable(settingheadertablemaxsize) {
        ;
    }
};
...
Hpack::Hpack(const size_t* const settingheadertablemaxsize): _hpack(new _Hpack(settingheadertablemaxsize)) {
    ;
}
HpackStatus Hpack::Encoder(HpWBuffer &stream,const std::vector<KeyValPair> &header)const {
    return _hpack->_Encoder<false, StringLiteralEncodeWay::EShortest>(stream, header, nullptr);
}
HpackStatus Hpack::Encoder(HpWBuffer &stream,const std::vector<KeyValPair> &header, const std::vector<std::string> &exclude)const {
    return _hpack->_Encoder<true, StringLiteralEncodeWay::EShortest>(stream, header, &exclude);
}
HpackStatus Hpack::Encoder(HpWBuffer &stream,const std::vector<KeyValPair> &header, std::vector<HeaderFieldInfo> &hfdilst)const {
    return _hpack->_EncoderHeaderFieldInfo<StringLiteralEncodeWay::EShortest>(stream, header, hfdilst);
}
HpackStatus Hpack::Decoder(HpRBuffer &stream, std::vector<KeyValPair> &header)const {
    return _hpack->_Decoder<false>(stream, header, nullptr);
}
HpackStatus Hpack::Decoder(HpRBuffer &stream, std::vector<KeyValPair> &header, std::vector<HeaderFieldInfo> &hfdilst)const {
    return _hpack->_Decoder<true>(stream, header, &hfdilst);
}
HpackStatus Hpack::Encoder(std::unique_ptr<HpWBuffer> stream,const std::vector<KeyValPair> &header)const {
    return _hpack->_Encoder<false, StringLiteralEncodeWay::EShortest>(*stream, header, nullptr);
}
HpackStatus Hpack::Encoder(std::unique_ptr<HpWBuffer> stream,const std::vector<KeyValPair> &header, const std::vector<std::string> &exclude)const {
    return _hpack->_Encoder<true, StringLiteralEncodeWay::EShortest>(*stream, header, &exclude);
}
HpackStatus Hpack::Encoder(std::unique_ptr<HpWBuffer> stream,const std::vector<KeyValPair> &header, std::vector<HeaderFieldInfo> &hfdilst)const {
    return _hpack->_EncoderHeaderFieldInfo<StringLiteralEncodeWay::EShortest>(*stream, header, hfdilst);
}
HpackStatus Hpack::Decoder(std::unique_ptr<HpRBuffer> stream, std::vector<KeyValPair> &header)const {
    return _hpack->_Decoder<false>(*stream, header, nullptr);
}
HpackStatus Hpack::Decoder(std::unique_ptr<HpRBuffer> stream, std::vector<KeyValPair> &header, std::vector<HeaderFieldInfo> &hfdilst)const {
    return _hpack->_Decoder<true>(*stream, header, &hfdilst);
}
HpackStatus Hpack::Encoder(HpWBuffer &stream,const std::vector<KeyValPair> &header, const StringLiteralEncodeWay SLEW)const {
    return _hpack->_Encoder<false>(stream, header, nullptr, SLEW);
}
HpackStatus Hpack::Encoder(HpWBuffer &stream,const std::vector<KeyValPair> &header, const std::vector<std::string> &exclude, const StringLiteralEncodeWay SLEW)const {
    return _hpack->_Encoder<true>(stream, header, &exclude, SLEW);
}
HpackStatus Hpack::Encoder(HpWBuffer &stream,const std::vector<KeyValPair> &header, std::vector<HeaderFieldInfo> &hfdilst, const StringLiteralEncodeWay SLEW)const {
    switch(SLEW) {
    case StringLiteralEncodeWay::EOctet:
        return _hpack->_EncoderHeaderFieldInfo<StringLiteralEncodeWay::EOctet>(stream, header, hfdilst);
    case StringLiteralEncodeWay::EHuffman:
        return _hpack->_EncoderHeaderFieldInfo<StringLiteralEncodeWay::EHuffman>(stream, header, hfdilst);
    case StringLiteralEncodeWay::EShortest:
        return _hpack->_EncoderHeaderFieldInfo<StringLiteralEncodeWay::EShortest>(stream, header, hfdilst);
    default:
        return HpackStatus::LiteralEncodeWay;
...
HpackStatus Hpack::EncoderDynamicTableSizeUpdate(HpWBuffer &stream,const size_t newSize, HeaderFieldInfo &hfdi) {
    return _hpack->_EncoderDynamicTableSizeUpdate(stream, newSize, hfdi);
}

std::unique_ptr<HpRBuffer> Hpack::MakeHpRBuffer(const std::vector<unsigned char> &ss) {
    return std::unique_ptr<HpRBuffer>(new VecHpRBuffer(ss));
}
std::unique_ptr<HpRBuffer> Hpack::MakeHpRBuffer(const unsigned char *start, size_t sizebyte) {
...
std::unique_ptr<HpWBuffer> Hpack::MakeHpWBuffer(std::vector<unsigned char> &ss, size_t maxsize) {
    return std::unique_ptr<HpWBuffer>(new VecHpWBuffer(ss, maxsize));
}
const std::string Hpack::HpackStatus2String(const HpackStatus &stat) {
    return std::string(HpackStatus2CString(stat));
}
const std::string Hpack::HeaderFieldRepr2String(const HeaderFieldRepresentation &hfrp) {
    return std::string(HeaderFieldRepr2CString(hfrp));
}
